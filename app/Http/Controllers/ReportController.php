...
//            PDF::loadHTML($html)->setPaper('a4', 'landscape')->setWarnings(false)->save('myfile.pdf')
    $pdf = PDF::loadView('reportes.reforma-pdf', compact('reforma', 'detalles_o', 'detalles_d'));
 ...
 */
public
function reformaSelectPDF(Request $request)
{
    ...
    $reforma = DB::table('reformas as r')
        ...
        ->join('programas as p', 'p.id', '=', 'ap.programa_id')
        ->join('actividads as a', 'a.id', '=', 'ap.actividad_id')
        ->select('p.programa', 'a.actividad', 'r.monto_orig', 'r.estado', 'r.tipo', 'ai.mes', 'i.cod_programa', 'i.cod_actividad', 'i.cod_item', 'i.item', 'i.grupo_gasto')
    ...
    $detalles_o = DB::table('reformas as r')
        ...
        ->join('actividads as a', 'a.id', '=', 'ap.actividad_id')
        ->join('pacs as pac', 'pac.id', '=', 'po.pac_id')
        ->select('po.valor_orig', 'p.programa', 'a.actividad', 'i.cod_programa', 'i.cod_actividad', 'i.cod_item', 'i.item', 'pac.mes')
        ->whereIn('r.id', $reformas_id)
//            ->where('r.id', $id)
        ->orderBy('r.id', 'desc')
        ->get();
    ...
    $detalles_d = DB::table('reformas as r')
        ...
        ->join('programas as p', 'p.id', '=', 'ap.programa_id')
        ->join('actividads as a', 'a.id', '=', 'ap.actividad_id')
        ->select('p.programa', 'a.actividad', 'i.cod_programa', 'i.cod_actividad', 'i.cod_item', 'i.item', 'pac.mes', 'pd.valor_dest')
//            ->groupBy('i.cod_programa','i.cod_actividad','i.cod_item')
        ->whereIn('r.id', $reformas_id)
        ->orderBy('pd.id', 'desc')
        ->get();
...
//Order By pd.reforma_id desc ;
    ...
    $todas = DB::table('pac_destino as pd')
        ->select('o.valor_orig', 'o.cod_programa as cod_programa_o', 'o.cod_actividad as cod_actividad_o', 'o.programa as programa_o', 'o.actividad as actividad_o', 'o.cod_item as cod_item_o', 'o.item as item_o', 'o.mes as mes_o', 'i.cod_programa', 'i.cod_actividad', 'p.programa', 'a.actividad', 'pac.cod_item', 'pac.item', 'pac.mes', 'pd.valor_dest')
        ->join('pacs as pac', 'pac.id', '=', 'pd.pac_id')
        ->join('area_item as ai', 'ai.id', '=', 'pac.area_item_id')
        ->join('items as i', 'i.id', '=', 'ai.item_id')
        ->join('programas as p', 'p.cod_programa', '=', 'i.cod_programa')
        ->join('actividads as a', 'a.cod_actividad', '=', 'i.cod_actividad')
        ->leftJoin(DB::raw('(SELECT po.id,po.reforma_id,po.pac_id,i.cod_programa,i.cod_actividad,p.programa,a.actividad,pac.cod_item,pac.item,pac.mes,po.valor_orig FROM pac_origen po join pacs pac on pac.id =po.pac_id join area_item ai on ai.id=pac.area_item_id join items i on i.id=ai.item_id join programas p on p.cod_programa=i.cod_programa join actividads a on a.cod_actividad=i.cod_actividad order by po.reforma_id) o'), function ($join) {
    ...
    $pdf = PDF::loadView('reportes.reforma_select_pdf', compact('reforma', 'detalles_o', 'detalles_d', 'total_reforma', 'todas'));
    return $pdf->setPaper('letter', 'landscape')->stream('Reforma');//imprime en pantalla
